"""B3 — FFB6D (He et al., CVPR 2021) Case R- RGB-only port.

ResNet-18 single-stream backbone + global pool + MLP → 14 × 3.
Architectural distinction: shallower backbone vs B1 (ResNet-34) and B2 (ResNet-50).
"""
...
class B3Ffb6dNet(nn.Module):
    def __init__(self):
        ...
        self.mlp = nn.Sequential(
            nn.Linear(flat, 768), nn.GELU(),
            nn.Linear(768, 768), nn.GELU(),
            nn.Linear(768, N_VERTICES * 3),
        )
